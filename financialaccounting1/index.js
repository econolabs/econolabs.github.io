...
function SimpleAccounting() {
    ...
    function handleAdd({ d, k, sum, bookD, bookK }) {
        dispatch({
            ...
            payload: {
                objects: {
                    content: [...records, { d, k, sum, bookD, bookK }]
                },
            },
        });
        updateRecords(
...
function SaveProject() {
    ...
    useEffect(() => {
        ...
        async function getUser() {
            ...
            if (Array.isArray(projectSelector.content) && projectSelector.content.length > 0) {
                let postObject = {
                    ...projectSelector,
                    comment: basicfirebasecrudservices.transactionsListFull(projectSelector.content.map(bookrecord => { return { ...bookrecord, period: "2025" } })),
                ...
                updates["currentDay/" + currentDay + "/posts/" + projectSelector.id + applicationSelector.userEmail + "media"] =
                {
                    ...
                    content: basicfirebasecrudservices.transactionsListFull(projectSelector.content.map(bookrecord => { return { ...bookrecord, period: "2025" } })),
                    type: "html",
                };
    ...
    }, [projectSelector.content.length])
    ...
    return <Navbar bg="light">
        ...
        <Form inline>
            <Button variant="outline-secondary" size="sm">{projectSelector.content.length}</Button>
        </Form>
